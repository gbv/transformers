...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <!-- TODO: add GND, Orte, Methoden(?) -->
    ...
    <xsl:variable name="getty" select="_nested__objekttyp__keywords_getty/objekttyp__keywords_getty"/>
    <xsl:if test="$getty">
      <datacite:subjects>
        <xsl:for-each select="$getty">
          <datacite:subject
            schemeURI="http://vocab.getty.edu/dataset/aat"
            valueURI="{custom/string[@name='conceptURI']}">
            <xsl:value-of select="custom/string[@name='conceptName']"/>
          </datacite:subject>
        </xsl:for-each>
      </datacite:subjects>
    </xsl:if>
  ...
  <xsl:template name="resourceType">
    <xsl:choose>
      <!-- Konvolut -->
     <xsl:when test="tags/tag[@id='1']"> <!-- TODO -->
